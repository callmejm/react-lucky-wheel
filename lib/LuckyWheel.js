...
var LuckyWheel = function (_Component) {
	...
	function LuckyWheel(props) {
		...
		_this.handleStop = function (position) {
			...
			// if (angle > 0) return;
			// console.log(position,areaNum);
			// var prizeAngle = position / areaNum * 360;
			var prizeAngle = 360 - (((position - 1) / areaNum) * 360);
		...
		_this.state = {
			angle: 0,
			TURNTABLE_BG:_this.props.TURNTABLE_BG,
			TURNTABLE:_this.props.TURNTABLE,
			POINTER:_this.props.POINTER,
			handleStart:_this.handleStart,
			renderData:_this.props.data,
			areaNum:_this.props.areaNum,
			wheelParentWidth:_this.props.wheelParentWidth,
	...
	createClass(LuckyWheel, [{
		...
		value: function render() {
			...
			var TURNTABLE_BG = this.state.TURNTABLE_BG;
			var TURNTABLE = this.state.TURNTABLE;
			var POINTER = this.state.POINTER;
			var renderData = this.state.renderData;
			var areaNum = this.state.areaNum;
			var wheelParentWidth = this.state.wheelParentWidth;
			var hideLabel = this.state.hideLabel;

			const ParentRadius = (wheelParentWidth * 0.8) / 2;
			const renderDatalength = renderData.length;
			const radiusList = Array.from(Array(renderDatalength).keys()).map(x => (360 / renderDatalength) * x);
			const positionPairList = radiusList.map(x => ({
			  x: Math.sin((Math.PI * x) / 180) * ParentRadius + 5,
			  y: Math.cos((Math.PI * x) / 180) * ParentRadius + 10
			}));
			return React.createElement(
				'div',
				{ className: 'c-wheel-content' },
				TURNTABLE_BG != null ? React.createElement('img', { className: '_bg',
					src: TURNTABLE_BG
				}) : null,
				TURNTABLE != null ? React.createElement('img', { className: '_turntable',
					style: {
						transform: 'rotate(' + angle + 'deg)'
					},
					src: TURNTABLE
				}) : null,
				POINTER != null ? React.createElement('img', { className: '_pointer',
					onClick: this.state.handleStart,
					src: POINTER
				}) : null,
				!hideLabel ?
				React.createElement('div', { className: '_data',
					...
					renderData.map((item, index) => {
						return React.createElement('div', { className: `_items`,
							style:{
								top:`${ParentRadius - positionPairList[index].y.toFixed(0)}px`,
								right:`${ParentRadius - positionPairList[index].x.toFixed(0)}px`,
